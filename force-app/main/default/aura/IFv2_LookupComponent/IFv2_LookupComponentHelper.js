({
    ...
    getRecordsBySearchTerm: function (component, event, helper) {
        var searchTerm = '';
        if (component.find('lookupInput') && component.find('lookupInput').getElement()) {
            searchTerm = component.find('lookupInput').getElement().value;
        }
        ...
        component.set('v.searchTimeout', setTimeout($A.getCallback(function () {
            ...
            getRecordsAction.setCallback(this, function (res) {
                if (res.getState() === 'SUCCESS') {
                    var returnValue = JSON.parse(res.getReturnValue());
                    if (component.find('lookupInput') !== undefined) {
                        if (component.find('lookupInput').getElement() !== null) {
                            if (returnValue !== undefined && returnValue !== null && returnValue.isSuccess && returnValue.results.searchTerm === component.find('lookupInput').getElement().value) {
                                var returnedRecords = [];
    ...
    addNewRecord: function (component, event, helper) {
        ...
        if (overrideNewEvent) {
            ...
            addRecordEvent.setParams({
                ...
                recordTypeId: component.get("v.recordType"), //Hot Fix Start W-010047
                navigationLocation: "LOOKUP", //Block redirect to created record detail page(Bug W-010047)
                //Hot Fix End
            });
        }
        addRecordEvent.fire();
